...
class _WikipediaExplorerState extends State<WikipediaExplorer> {
  ...
  void initState() {
    super.initState();
    //new MethodChannel('app.channel.shared.data').setMethodCallHandler(handler)
    getSharedText();
    print("initState()");
  }
  ...
  getSharedText() async {
    ...
    var sharedData = await platform.invokeMethod("helloFromNativeCode");
      print("sharedData=>" + sharedData);
    if (sharedData != null) {
      _controller.future.then((result){
        print("_controller.then()");
        result.loadUrl(File(sharedData).toString());
      });
    }
  }
